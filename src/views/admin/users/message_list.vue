<template>
  <div class="message_list_view">
    <div class="user_list_menu">
      <div class="head">
        <a-input-search placeholder="搜索用户名" v-model="params.nickName" @search="getMessageData"
                        @keydown.enter="getMessageData"></a-input-search>
      </div>
      <gvb_message_list id-key="send_user_id" @check="messageCheck" :data="messageData.list"></gvb_message_list>

      <div class="page">
        <a-pagination simple :total="messageData.count" @change="getMessageData" v-model:current="params.page"
                      :page-size="params.limit"></a-pagination>
      </div>
    </div>
    <div class="user_menu" v-if="messageUserData.list.length">
      <gvb_message_list id-key="rev_user_id" @check="messageUserCheck" :data="messageUserData.list"></gvb_message_list>
    </div>
...
watch(() => route.query.send_user_id, () => {
